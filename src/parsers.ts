...
import {
    Technology, CardData, Email, Location, SocialLink,
} from './types.js';
...
export function parseTechnologyContainer(
    ...
    const results: Record<string, Technology[]> = {};
    ...
    categoryCards.each((__, cardNode) => {
        ...
        const technologies: { name: string; version?: string }[] = [];
...
export function parserInfoContainer(infoHtml: string) {
    ...
    const results: Record<string, CardData | string[]> = {};
        ...
        subCategoryElements.each((__, subCategoryNode) => {
            ...
            switch (subCategoryHeader) {
                ...
                case 'LinkedIn': {
                    ...
                    cardData[subCategoryHeader] = socialLinks.length === 1 ? socialLinks[0] : socialLinks;
                    break;
                }

                case 'SSL/TLS enabled': {
                    const isEnabled = subCategoryContentElement.find('span.v-icon.success--text').length > 0;
